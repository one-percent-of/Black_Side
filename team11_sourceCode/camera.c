...
#define  IMAGE_FILE "/home/root/pic.img"
...
//원하는 위치부터 camera의 사이즈만큼 displayFrame에 저장
void DrawFromRGB565(unsigned char *displayFrame, unsigned char *videoFrame, int d_x, int d_y, int videoWidth, int videoHeight, int dFrameWidth, int dFrameHeight, int bmp_generator_use) {
	...
	if(bmp_generator_use){
		videoptr = videoFrame;
   		for( y=0; y < videoHeight; y++){
			for(x = 0; x< videoWidth; x++){

				videptrTemp = videoptr + videoWidth*y +x ;
				r = (unsigned char)((*videptrTemp&0xf800) >>8);
				g = (unsigned char)((*videptrTemp&0x07E0) >>3);
				b = (unsigned char)((*videptrTemp&0x001F) <<3);

				data[y*1280+x].rgbRed = r;
				data[y*1280+x].rgbGreen = g;
				data[y*1280+x].rgbBlue = b;
		   	}
	   	}
	 	bmp_generator(bmp_file, 1280, 960, data);
	 }
}

static void DrawFromRGB565_1(unsigned char *displayFrame, unsigned char *videoFrame,int videoWidth, int videoHeight, \
				 int dFrameWidth,int dFrameHeight,int bmp_generator_use)
{
	int    x,y;
	int lineLeng ;
	unsigned short *videptrTemp;
   ...
   for( y=0; y < videoHeight; y++){
	for(x = 0; x< videoWidth; x++){
		...
		data[y*640+x].rgbRed = r;
		data[y*640+x].rgbGreen = g;
		data[y*640+x].rgbBlue = b;
	   }
   }
 if(bmp_generator_use){
 	bmp_generator(bmp_file, 640, 480, data);
 }
}
#define  FBDEV_FILE "/dev/fb0"

int FrameInit(int* afb_fd, struct fb_var_screeninfo* afbvar,
struct fb_fix_screeninfo* afbfix)
{
	if (access(FBDEV_FILE, F_OK)) {
		printf("%s: access error\n", FBDEV_FILE);
		return 0;
	}
	if ((*afb_fd = open(FBDEV_FILE, O_RDWR)) < 0) {
		printf("%s: open error\n", FBDEV_FILE);
		return 0;
	}
	if (ioctl(*afb_fd, FBIOGET_VSCREENINFO, afbvar)) {
		printf("%s: ioctl error - FBIOGET_VSCREENINFO \n", FBDEV_FILE);
		close(*afb_fd);
		exit(1);
	}
	if (ioctl(*afb_fd, FBIOGET_FSCREENINFO, afbfix)) {
		printf("%s: ioctl error - FBIOGET_FSCREENINFO \n", FBDEV_FILE);
		close(*afb_fd);
		exit(1);
	}

	screen_width = afbvar->xres;
	screen_height = afbvar->yres;
	bits_per_pixel = afbvar->bits_per_pixel;
	line_length = afbfix->line_length;
...
int bmp_generator(char *filename, int width, int height, RGBQUAD *data) { // 저장된 사진을 bmp파일로 변환 (7조가 도움줌)
	printf("GENERATOR START\n");
	BITMAPFILEHEADER1 bmp_head;
	BITMAPINFOHEADER1 bmp_info;
	int size = width * height * 4;
	printf("bmp");
	bmp_head.bfType = 0x4D42; // 'BM'
	bmp_head.bfSize = size + sizeof(BITMAPFILEHEADER1) + sizeof(BITMAPINFOHEADER1); // 24 + head + info no quad
	...
	printf("bmp");
	fwrite(&bmp_head, 1, sizeof(BITMAPFILEHEADER1), fp);
	fwrite(&bmp_info, 1, sizeof(BITMAPINFOHEADER1), fp);
...
int camera(unsigned char *buffer, int d_x, int d_y) // 실시간 카메라 사진 띄움
{	
	int     fb_fd;
	int	    ret = 0;
	int     index;

	fb_fd = CreateCamera(0);
	if (fb_fd <= 0) {
		perror("\nCamera open Error\n");
		exit(0);
	}
	...
	while (1)
	{
		...
		DrawFromRGB565(buffer, m_buffers_preview[index].virt.p, d_x,d_y, CAMERA_PREVIEW_WIDTH,CAMERA_PREVIEW_HEIGHT, 1280, 960,0);
		if (gap >= 10) break; // 10초간 후방카메라 작동
...
int camera1() // 2초후 사진찍음
{
	...
	unsigned char	*img_mapped; //코딩 추가한 부분
	int             mem_size;
	
	char bmp_name[30];
	char *bmp1="/home/root/";
	char *bmp2=".bmp";
	sprintf(bmp_name, "%s%s%s\0",bmp1,"pic",bmp2);
	bmp_file = bmp_name;
	printf("%s\n",bmp_name);
	...
	fb_ptr   =   mmap(0, mem_size,
		 PROT_READ|PROT_WRITE, MAP_SHARED, fb_fd, 0);
	fb_mapped = (unsigned char *)fb_ptr;
	if (fb_mapped < 0)
	{
		printf("fb_mapped mmap error!\n");
	...
	while(1){
		...
		ret = fimc_poll(&m_events_c);
		CHECK_PTR(ret);
		buf_index = fimc_v4l2_dqbuf(m_cam_fd, 1);
		DrawFromRGB565_1((unsigned char *)fb_ptr, m_buffers_preview[buf_index].virt.p,CAMERA_PREVIEW_WIDTH,CAMERA_PREVIEW_HEIGHT,screen_width,screen_height,0);
		if (gap >= 2) break; // 카메라가 움직이기 시작했을때 2초후에 사진 저장
		ret = fimc_v4l2_qbuf(m_cam_fd,buf_index); //새로운 프레임을 달라고 요청
	}
	printf("cap done!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	...
	if( (img_fd = open(IMAGE_FILE, O_RDWR | O_CREAT, 0777))<0){
		printf("%s: open error\n", IMAGE_FILE);
		return 0;
	}

	img_mapped = (unsigned char *)mmap(0, mem_size, PROT_READ|PROT_WRITE, MAP_SHARED, img_fd, 0);
	write(img_fd,(void*)m_buffers_preview[buf_index].virt.p,mem_size);
  ...
  if( (img_fd = open(IMAGE_FILE, O_RDWR))<0){
    printf("%s: open error\n", IMAGE_FILE);
    printf("img open\n");
  }

  //img_mapped	=	(unsigned char *)mmap(0, mem_size, PROT_READ|PROT_WRITE, MAP_SHARED, img_fd, 0);
  DrawFromRGB565_1(fb_mapped, img_mapped ,CAMERA_PREVIEW_WIDTH,CAMERA_PREVIEW_HEIGHT,screen_width,screen_height,1);

	

	printf("camera capture sucess.\n");
	stopPreview();
	usleep(1000);
	DestroyCamera();
	close(img_fd);
